...
const setShaderRegisteredCallback = process.browser && require('webgpu-live-shader-module').setShaderRegisteredCallback;
...
class CanvasRAFWhileMounted extends React.Component<JSX.IntrinsicElements['canvas'] & {
  ...
  componentDidMount() {
    ...
    setShaderRegisteredCallback(async (source, updatedSource) => {
      ...
      this.props.addShaderEditor(
        <div className={styles.shaderEditor} key={source} ref={(el) => {
          if (!el) return;
    ...
    this.props.init(this.canvasRef.current, this.props.useWGSL, gui).then(frame => {
...
export function makeBasicExample(props: {
  ...
  return () => {
    ...
    const notWritten = shaders ? null : (
      <>
        <br /><br />
        Sorry, this example hasn't been written for {useWGSL ? 'WGSL' : 'GLSL'} yet.
      </>
    );
      ...
      <main>
        ...
        <div>
          ...
          {supported ? null : <>
            ...
            <p>WebGPU or this example is not supported! Please visit <a href="//webgpu.io">webgpu.io</a> to see the current implementation status.</p>
          </>}
          {notWritten}
        </div>
