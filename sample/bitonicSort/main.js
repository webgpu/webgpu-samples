...
class Base2DRendererClass {
    renderPassDescriptor;
    pipeline;
    bindGroupMap;
    currentBindGroup;
    currentBindGroupName;
    executeRun(commandEncoder, renderPassDescriptor, pipeline, bindGroups) {
    ...
    setUniformArguments(device, uniformBuffer, instance, keys) {
        for (let i = 0; i < keys.length; i++) {
            device.queue.writeBuffer(uniformBuffer, i * 4, new Float32Array([instance[keys[i]]]));
        }
    }
    create2DRenderPipeline(device, label, bgLayouts, code, presentationFormat) {
...
var bitonicDisplay = `struct ComputeUniforms {
  width: f32,
  height: f32,
  algo: u32,
  blockHeight: u32,
...
class BitonicDisplayRenderer extends Base2DRendererClass {
    ...
    setArguments;
    computeBGDescript;
    constructor(device, presentationFormat, renderPassDescriptor, computeBGDescript, label) {
        super();
        ...
        const bgCluster = createBindGroupCluster([0], [GPUShaderStage.FRAGMENT], ['buffer'], [{ type: 'uniform' }], [[{ buffer: uniformBuffer }]], label, device);
        this.currentBindGroup = bgCluster.bindGroups[0];
        this.pipeline = super.create2DRenderPipeline(device, label, [this.computeBGDescript.bindGroupLayout, bgCluster.bindGroupLayout], bitonicDisplay, presentationFormat);
        this.setArguments = (args) => {
            device.queue.writeBuffer(uniformBuffer, 0, new Uint32Array([args.highlight]));
        };
    }
    startRun(commandEncoder, args) {
        this.setArguments(args);
...
const NaiveBitonicCompute = (workgroupSize) => {
...
var atomicToZero = `@group(0) @binding(3) var<storage, read_write> counter: atomic<u32>;
...
var StepEnum;
(function (StepEnum) {
    StepEnum[StepEnum["NONE"] = 0] = "NONE";
    StepEnum[StepEnum["FLIP_LOCAL"] = 1] = "FLIP_LOCAL";
    StepEnum[StepEnum["DISPERSE_LOCAL"] = 2] = "DISPERSE_LOCAL";
    StepEnum[StepEnum["FLIP_GLOBAL"] = 3] = "FLIP_GLOBAL";
    StepEnum[StepEnum["DISPERSE_GLOBAL"] = 4] = "DISPERSE_GLOBAL";
})(StepEnum || (StepEnum = {}));
const getNumSteps = (numElements) => {
    const n = Math.log2(numElements);
    return (n * (n + 1)) / 2;
};
SampleInitFactoryWebGPU(async ({ device, gui, presentationFormat, context, canvas, timestampQueryAvailable, }) => {
    const maxInvocationsX = device.limits.maxComputeWorkgroupSizeX;
    let querySet;
    let timestampQueryResolveBuffer;
    let timestampQueryResultBuffer;
    if (timestampQueryAvailable) {
    ...
    const defaultGridWidth = Math.sqrt(maxElements) % 2 === 0
    ...
    let elements = new Uint32Array(Array.from({ length: settings['Total Elements'] }, (_, i) => i));
    // Initialize elementsBuffer and elementsStagingBuffer
    const elementsBufferSize = Float32Array.BYTES_PER_ELEMENT * totalElementOptions[0];
    ...
    const computeBGCluster = createBindGroupCluster([0, 1, 2, 3], [
    ...
    const bitonicDisplayRenderer = new BitonicDisplayRenderer(device, presentationFormat, renderPassDescriptor, computeBGCluster, 'BitonicDisplay');
    const resetTimeInfo = () => {
        settings.stepTime = 0;
        settings.sortTime = 0;
        stepTimeController.setValue('0ms');
        sortTimeController.setValue(`0ms`);
        const nanCheck = settings.configToCompleteSwapsMap[settings.configKey].time /
    ...
    const resetExecutionInformation = () => {
        ...
        workgroupSizeController.setValue(Math.min(settings['Total Elements'] / 2, settings['Size Limit']));
        // Dispatch a workgroup for every (Size Limit * 2) elements
        const workgroupsPerStep = (settings['Total Elements'] - 1) / (settings['Size Limit'] * 2);
        ...
        const newCellWidth = Math.sqrt(settings['Total Elements']) % 2 === 0
    ...
    const resizeElementArray = () => {
        ...
        elements = new Uint32Array(Array.from({ length: settings['Total Elements'] }, (_, i) => i));
    ...
    const sizeLimitController = computeResourcesFolder
        ...
        const constraint = Math.min(settings['Total Elements'] / 2, settings['Size Limit']);
        const workgroupsPerStep = (settings['Total Elements'] - 1) / (settings['Size Limit'] * 2);
    ...
    const workgroupSizeController = computeResourcesFolder.add(settings, 'Workgroup Size');
    const workgroupsPerStepController = computeResourcesFolder.add(settings, 'Workgroups Per Step');
    ...
    const currentStepController = executionInformationFolder.add(settings, 'Current Step');
    const prevStepController = executionInformationFolder.add(settings, 'Prev Step');
    const nextStepController = executionInformationFolder.add(settings, 'Next Step');
    ...
    for (let i = 0; i < liFunctionElements.length; i++) {
        liFunctionElements[i].children[0].style.display = 'flex';
        liFunctionElements[i].children[0].style.justifyContent =
            'center';
        liFunctionElements[i].children[0].children[1].style.position = 'absolute';
    }
    ...
    canvas.addEventListener('mousemove', (event) => {
        ...
        const yIndex = settings['Grid Height'] - 1 - Math.floor(event.offsetY / cellSize[1]);
    ...
    async function frame() {
        ...
        device.queue.writeBuffer(elementsInputBuffer, 0, elements.buffer, elements.byteOffset, elements.byteLength);
        ...
        device.queue.writeBuffer(computeUniformsBuffer, 0, dims.buffer, dims.byteOffset, dims.byteLength);
        ...
        if (settings.executeStep &&
            ...
            await elementsStagingBuffer.mapAsync(GPUMapMode.READ, 0, elementsBufferSize);
            const copyElementsBuffer = elementsStagingBuffer.getMappedRange(0, elementsBufferSize);
            // Copy atomic swaps data to CPU
            await atomicSwapsStagingBuffer.mapAsync(GPUMapMode.READ, 0, Uint32Array.BYTES_PER_ELEMENT);
            const copySwapsBuffer = atomicSwapsStagingBuffer.getMappedRange(0, Uint32Array.BYTES_PER_ELEMENT);
            const elementsData = copyElementsBuffer.slice(0, Uint32Array.BYTES_PER_ELEMENT * settings['Total Elements']);
            const swapsData = copySwapsBuffer.slice(0, Uint32Array.BYTES_PER_ELEMENT);
            // Extract data
            ...
            if (timestampQueryAvailable) {
                ...
                await timestampQueryResultBuffer.mapAsync(GPUMapMode.READ, 0, 2 * BigInt64Array.BYTES_PER_ELEMENT);
                ...
                const newStepTime = Number(copyTimestampResult[1] - copyTimestampResult[0]) / 1000000;
                ...
                if (highestBlockHeight === settings['Total Elements'] * 2) {
                    ...
                    settings.configToCompleteSwapsMap[settings.configKey].time +=
                        newSortTime;
                    const averageSortTime = settings.configToCompleteSwapsMap[settings.configKey].time /
                        settings.configToCompleteSwapsMap[settings.configKey].sorts;
                    averageSortTimeController.setValue(`${averageSortTime.toFixed(5)}ms`);
                }
