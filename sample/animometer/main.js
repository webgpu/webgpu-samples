...
var animometerWGSL = `struct Time {
  value : f32,
}
...
function configure() {
    ...
    const uniformBuffer = device.createBuffer({
        size: Math.max(numTriangles, 1) * alignedUniformBytes +
    ...
    for (let offset = 0; offset < uniformBufferData.length; offset += maxMappingLength) {
        ...
        device.queue.writeBuffer(uniformBuffer, offset * Float32Array.BYTES_PER_ELEMENT, uniformBufferData.buffer, uniformBufferData.byteOffset + offset * Float32Array.BYTES_PER_ELEMENT, uploadCount * Float32Array.BYTES_PER_ELEMENT);
    }
